...
def data_extractor():
    ...
    columns = request.args.get("columns", "*")  # Default to all columns if not specified
    group_by = request.args.get("group_by", "")  # Optional GROUP BY clause
    ...
    try:
        ...
        data_from_table = pd.read_sql(query, conn)
